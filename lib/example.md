...
[*\# leave blank for output to stdout:*]{style="color: 0.38,0.63,0.69"}
...
[*\# Note that these take files, not their
...
[data-dir]{style="color: 0.02,0.16,0.49"}[**:**]{style="color: 0.00,0.44,0.13"}
...
[title-prefix]{style="color: 0.02,0.16,0.49"}[**:**]{style="color: 0.00,0.44,0.13"}[
...
[epub-subdirectory]{style="color: 0.02,0.16,0.49"}[**:**]{style="color: 0.00,0.44,0.13"}[
...
:   when used in conjunction with `pdfa`, specifies the ICC profile to
    use in the PDF, e.g. `default.cmyk`. If left unspecified, `sRGB.icc`
    is used as default. May be repeated to include multiple profiles.
    Note that the profiles have to be available on the system. They can
    be obtained from [ConTeXt ICC
...
-   [`raw_tex`](#extension-raw_tex) allows raw LaTeX, TeX, and ConTeXt
    ...
    richest possible outputs when several options are given, you will
    get best results if you disable `raw_html` and `raw_tex` when
    converting to formats like `docx` which don't allow raw `html` or
    `tex`.

-   [`native_divs`](#extension-native_divs) causes HTML `div` elements
...
-   [`native_spans`](#extension-native_spans) causes HTML `span`
    elements to be parsed as native pandoc Span inlines. If you want
    them to be parsed as raw HTML, use `-f html-native_spans+raw_html`.
...
body. (For example `----` would make the first column 3/4 and the second
...
A [YAML](https://yaml.org/spec/1.2/spec.html) metadata block is a valid
YAML object, delimited by a line of three hyphens (`---`) at the top and
a line of three hyphens (`---`) or three dots (`...`) at the bottom. A
YAML metadata block may occur anywhere in the document, but if it is not
at the beginning, it must be preceded by a blank line. (Note that,
because of the way pandoc concatenates input files when several are
provided, you may also keep the metadata in a separate YAML file and
...
Note that [YAML](https://yaml.org/spec/1.2/spec.html) escaping rules
...
considered part of the key. In `@Foo_bar.baz.`, the key is
`Foo_bar.baz`. The final period is not *internal* punctuation, so it is
not included in the key. In `@{Foo_bar.baz.}`, the key is
`Foo_bar.baz.`, including the final period. The curly braces are
...
:   The `lang` field will affect how the style is localized, for example
    ...
    be used to specify options for collation (sorting) more precisely.
    Here are some examples:
...
show using LaTeX [`beamer`](https://ctan.org/pkg/beamer), or slides
shows in Microsoft
...
`t`, `environment`, `label`, `plain`, `shrink`, `standout`,
`noframenumbering`.
...
See the [reveal.js documentation](https://revealjs.com/backgrounds/) for
more details.
...
:   Either a string value, or an object with fields `role`, `file-as`,
    ...
    [MARC relators](https://loc.gov/marc/relators/relaterm.html), but
...
:   A string value in [BCP 47](https://tools.ietf.org/html/bcp47)
...
the [GPL](https://www.gnu.org/copyleft/gpl.html), version 2 or greater.
